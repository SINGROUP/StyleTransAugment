...
\begin{document}
	...
	\begin{tikzpicture}[node distance = 5cm, on grid, auto]
		...
		\node at  ($(U.south)+(-0.25,-1cm)$) {\footnotesize
			{\textcolor{dftcolor}{Cycle 1}:} $u \textcolor{dftcolor}{\rightarrow}  G_\mathcal{U} \textcolor{dftcolor}{\rightarrow} \tilde{v} \textcolor{dftcolor}{\rightarrow} G_\mathcal{V} \textcolor{dftcolor}{\rightarrow} \hat{u}$};
		...
		\node at  ($(V.south)+(+0.15,-1cm)$) {\footnotesize
			{\textcolor{black}{Cycle} 2:} $v \textcolor{black}{\rightarrow} G_\mathcal{V} \textcolor{black}{\rightarrow} \tilde{u} \textcolor{black}{\rightarrow} G_\mathcal{U} \textcolor{black}{\rightarrow} \hat{v}$};
		...
		\path[transfer, dftcolor] (u) edge (gu);
		\path[transfer, dftcolor] (gu) edge (vt);
		\path[transfer, dashed] (vt) edge [bend left] (dv);  % adversarial loss
		\path[transfer, dashed] (v) edge [bend right] (dv); % adversarial loss
		\path[transfer, dftcolor] (vt) edge [bend right] (gv); % recover
		\path[transfer, dftcolor] (gv) edge [bend right=25] ($(uh.east)+(0,-0.1cm)$); % recover
	...
	\begin{scope}[shift={([xshift=-0.4cm,yshift=-2.8cm]du.south)}]
		...
		inner sep=2pt
			...
			\hspace{0.2em}
			\tikz[baseline=-2.0ex] \node(R) [real, rounded corners=0pt, minimum size=0.4cm] {}; \hspace{0em} Source image
			\hspace{0.2em}
