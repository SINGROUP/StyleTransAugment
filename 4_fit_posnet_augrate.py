...
def apply_preprocessing(batch, cfg, gen_ab=None):
    ...
    pp.style_translate(X, gen_ab, cfg['rank'], aug_rate=cfg['aug_rate']) if gen_ab is not None else None
    pp.add_norm(X)
...
def run(rank, cfg, gen_ab):
    ...
    os.environ['MASTER_PORT'] = '{}'.format(random.randint(10000, 20000) # Random port
